...
//             bName: orderSnapshot['bName'] as String? ?? '',
//             bAddress: orderSnapshot['bAddress'] as String? ?? '',
...
class _OrdersTabState extends State<OrdersTab> {
  ...
  // separate the previous and ongoing orders
  void getOrdersDetails() async {
    try {
      List<String> orderIds = await fetchOrderIdsForUser(userId);
      List<MyOrder> orders = await fetchOrdersDetails(orderIds);
  ...
  Future<List<String>> fetchOrderIdsForUser(String userId) async {
    try {
      ...
      if (userSnapshot.exists && userSnapshot.data() != null) {
        List<dynamic> orderIds = (userSnapshot.data() as Map<String, dynamic>)['OrdersHistory'] ?? [];
        print(orderIds);
        return List<String>.from(orderIds);
      }
      ...
      try {
        ...
        if (orderSnapshot.exists && orderSnapshot.data() != null) {
          ...
          MyOrder order = MyOrder(
            ...
            bName: orderData['bName'] ?? '',
            bAddress: orderData['bAddress'] ?? '',
            totalValue: orderData['totalValue'] ?? 0,
            createdTimestamp: orderData['createdTimestamp'] ?? Timestamp.now(),
            deliveredTimestamp: orderData['deliveredTimestamp'],
            cancelledTimestamp: orderData['cancelledTimestamp'],
            tag: orderData['tag'] ?? '',
